%This is a MATALB script that leanrs the controlleed joint dynamics of a
%2DoF using ELM with control barrier and Lyapunov functions. 
...
%% Preamble
[demos,Data,Inside,Out, center, theta,mjr_axis, mnr_axis,D,xstar,tau] = dataprep(); %2DoF self generated data
...
for m = 1:nbPoints
    ...
    for j=1:length(Reference) 
        NonLinDemo_repro(:,j,m) = feval(NonLin,estimatePos(1,j),estimatePos(2,j),...
                                                estimatePos(1,j)-params.xstar(1),estimatePos(2,j)-params.xstar(2));%,
